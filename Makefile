...
PRJSRC=avr.c include/lcd_lib.c system.c uart.c suart.c rtc.c adc.c config.c
...
# compiler
CFLAGS=-I. $(INC) -DF_CPU=3686400 -g -mmcu=$(MCU) -O$(OPTLEVEL) \
...
# c++ specific flags
CPPFLAGS=-DF_CPU=3686400 -fno-exceptions               \
	-Wa,-ahlms=$(firstword         \
	$(filter %.lst, $(<:.cpp=.lst))\
	$(filter %.lst, $(<:.cc=.lst)) \
	$(filter %.lst, $(<:.C=.lst)))
